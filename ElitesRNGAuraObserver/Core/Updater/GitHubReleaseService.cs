...
internal class GitHubReleaseService : IDisposable
{
    ...
    public async Task<ReleaseInfo> GetLatestReleaseAsync(string assetName)
    {
        ...
        JsonNode? obj = JsonNode.Parse(json) ?? throw new InvalidOperationException("Failed to parse release information");
        ...
        var assetUrl = obj["assets"]?.AsArray()
            ?.FirstOrDefault(x => string.Equals(x?["name"]?.ToString(), assetName, StringComparison.Ordinal))?["browser_download_url"]?.ToString();
