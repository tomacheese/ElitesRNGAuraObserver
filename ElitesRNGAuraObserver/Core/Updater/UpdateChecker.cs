...
internal class UpdateChecker(GitHubReleaseService gh)
{
    ...
    {
        _latest = await gh.GetLatestReleaseAsync("ElitesRNGAuraObserver.zip").ConfigureAwait(false);
        return _latest;
    }
    ...
    public static async Task<bool> CheckAsync()
    {
        try
        {
            ...
            var updaterPath = Path.Combine(Application.StartupPath, "ElitesRNGAuraObserver.Updater.exe");
            ...
            var assetName = "ElitesRNGAuraObserver.zip";
        ...
        {
            await Console.Error.WriteLineAsync($"File not found: {ex.Message}").ConfigureAwait(false);
            await Console.Error.WriteLineAsync(ex.StackTrace).ConfigureAwait(false);
            return false;
        }
        catch (InvalidOperationException ex)
        {
            await Console.Error.WriteLineAsync($"Invalid operation: {ex.Message}").ConfigureAwait(false);
            await Console.Error.WriteLineAsync(ex.StackTrace).ConfigureAwait(false);
            return false;
        }
        catch (Exception ex)
