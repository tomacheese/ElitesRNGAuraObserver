using ElitesRNGAuraObserver.Core.Aura;
using ElitesRNGAuraObserver.Core.Config;
using ElitesRNGAuraObserver.Core.Notification;
...
internal class AuraObserverController(ConfigData configData) : IDisposable
{
    ...
    private void OnNewAuraDetected(Aura.Aura aura, bool isFirstReading)
    {
        ...
        if (isFirstReading || aura.Category == AuraCategory.AuraCategories.Ordinary)
        {
            return;
        }
